document.addEventListener("DOMContentLoaded", () => {
        ...
        window.electron
          ...
          .then((envVars) => {
            document.getElementById("apiKey").value =
              envVars.CAPITAL_API_KEY || "";
            document.getElementById("apiPassword").value =
              envVars.CAPITAL_API_PASSWORD || "";
            document.getElementById("apiIdentifier").value =
              envVars.CAPITAL_API_IDENTIFIER || "";
          });
      }
    }
  });
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const connected = await connectToCapital(
        apiKey,
        apiPassword,
        apiIdentifier,
      );
  ...
  async function connectToCapital(apiKey, apiPassword, apiIdentifier) {
    try {
      const success = await window.capitalApi.connect(
        apiKey,
        apiPassword,
        apiIdentifier,
      );
  ...
  async function fetchPositionsAndOrders() {
    ...
    try {
      ...
      // Fetch signals
      const signals = await window.capitalApi.getSignals(
        appState.currentSymbol,
        appState.currentInterval,
      );
  ...
  function isScalpingTrade(position) {
    ...
    return (
      position.timeframe &&
      ["1m", "5m", "15m", "30m"].includes(position.timeframe)
    );
  }
  ...
  function renderPositions(positions) {
    ...
    positions.sort(
      (a, b) => parseFloat(b.profitPercent) - parseFloat(a.profitPercent),
    );
  ...
  function renderSignals(signals) {
    ...
    if (!signals || signals.length === 0) {
      signalsList.innerHTML =
        '<div class="empty-message">No trading signals</div>';
      return;
    }
  ...
  async function executeTrade(signal) {
    ...
    try {
      showNotification(
        `Executing ${signal.direction} trade for ${signal.symbol}...`,
        "info",
      );
      ...
      const position = {
        ...
        stopLevel:
          parseFloat(signal.price) * (signal.direction === "BUY" ? 0.98 : 1.02),
        profitLevel:
          parseFloat(signal.price) * (signal.direction === "BUY" ? 1.05 : 0.95),
      ...
      showNotification(
        `${signal.direction} trade executed for ${signal.symbol}`,
        "success",
      );
    } catch (error) {
  ...
  async function closePosition(dealId) {
    ...
    try {
      ...
      const positionIndex = appState.positions.findIndex(
        (p) => p.dealId === dealId,
      );
  ...
  function updatePortfolioValue() {
    ...
    appState.accountInfo.dailyChange = (
      parseFloat(appState.accountInfo.dailyChange) +
      (change / currentBalance) * 100
    ).toFixed(2);
    ...
    portfolioChange.className =
      appState.accountInfo.dailyChange >= 0 ? "green" : "red";
  }
  ...
  function generateRandomPosition() {
    ...
    return {
      ...
      currentPrice: (
        entryPrice +
        (direction === "BUY" ? 1 : -1) * entryPrice * 0.01
      ).toFixed(2),
      stopLevel: (
        entryPrice -
        (direction === "BUY" ? 1 : -1) * entryPrice * 0.02
      ).toFixed(2),
      profitLevel: (
        entryPrice +
        (direction === "BUY" ? 1 : -1) * entryPrice * 0.03
      ).toFixed(2),
  ...
  function simulateMarketUpdates() {
    ...
    if (appState.connected && appState.botRunning) {
      ...
      if (dailyChangeEl) {
        ...
        dailyChangeEl.className = isPositive
          ? "positive-change"
          : "negative-change";
      ...
      if (appState.positions.length > 0) {
        appState.positions.forEach((position) => {
          ...
          const priceChange =
            (Math.random() - 0.45) * (parseFloat(position.entryPrice) * 0.01); // Slightly biased towards profit
          position.currentPrice = (
            parseFloat(position.currentPrice) + priceChange
          ).toFixed(2);

          // Calculate profit
          const priceDirection = position.direction === "BUY" ? 1 : -1;
          ...
          position.profitPercent = (
            (priceDiff / parseFloat(position.entryPrice)) *
            100
          ).toFixed(2);
        });
        ...
        if (Math.random() < 0.05 && appState.positions.length > 0) {
          const randomIndex = Math.floor(
            Math.random() * appState.positions.length,
          );
      ...
      if (Math.random() < 0.15 && appState.signals.length > 0) {
        const randomSignalIndex = Math.floor(
          Math.random() * appState.signals.length,
        );
