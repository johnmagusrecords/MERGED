...
function initializeApp() {
  ...
  // Check for saved credentials via Electron IPC if available
  if (window.electron) {
    console.log("Electron detected, checking for saved credentials...");
    window.electron.send("get-credentials");

    // Listen for credentials response
    window.electron.receive("credentials", (credentials) => {
      if (credentials) {
        console.log("Credentials found, logging in...");
        appState.user = {
          username: credentials.identifier || "User",
    ...
    // Default to login screen if not in Electron
    setTimeout(() => {
      console.log("No Electron context, showing login screen...");
      initializeLoginScreen();
    }, 2000);
  }
}
...
function handleLogin() {
  ...
  // Save credentials via Electron IPC if available
  if (window.electron) {
    const credentials = {
      identifier: username,
      password: password,
    ...
    window.electron.send("save-credentials", credentials);
  }
...
function initializeChartAnalyzer() {
  ...
  appState.chartAnalyzer = {
    ...
    indicators: {
      movingAverages: {
        ...
        calculate: function (data, type, period) {
          ...
          console.log(
            `Calculating ${type}${period} for ${appState.activeSymbol}`,
          );
      ...
      oscillators: {
        ...
        calculate: function (data) {
          ...
          this.MACD.values.histogram =
            this.MACD.values.line - this.MACD.values.signal;
          ...
          if (this.MACD.values.histogram > 0 && this.MACD.values.line > 0) {
            ...
            this.MACD.values.histogram < 0 &&
            this.MACD.values.line < 0
          ) {
    ...
    candlestickPatterns: {
      ...
      detect: function (data) {
        ...
        if (this.patterns[randomIndex].name === "Engulfing") {
          this.patterns[randomIndex].direction =
      ...
      detect: function (data) {
        ...
        if (Math.random() > 0.7) {
          ...
          if (this.patterns[randomIndex].name === "Triangle") {
            ...
            this.patterns[randomIndex].type =
    ...
    analyze_market: function (symbol, timeframe) {
      ...
      if (
        this.indicators.oscillators.RSI.signal === "oversold" &&
        this.indicators.oscillators.MACD.indication === "bullish"
      ) {
        oscillatorSignal = "bullish";
      } else if (
        this.indicators.oscillators.RSI.signal === "overbought" &&
...
function initializeCapitalComTrading() {
  ...
  appState.brokerConnection = {
    ...
    placeOrder: function (orderType, symbol, amount, leverage = 30) {
      console.log(
        `Placing ${orderType} order for ${symbol}, Amount: ${amount}, Leverage: ${leverage}`,
      );
  ...
  if (username && apiKey) {
    ...
    console.warn(
      "No Capital.com credentials found. Trading features will be limited.",
    );
  }
...
function displayAnalysisResults(results) {
  ...
  const overallSignal = results.signal || {
    direction: "neutral",
    strength: 50,
    recommendation: "Hold",
  };
  ...
  const signalClass =
    overallSignal.direction === "buy"
      ? "positive"
      : overallSignal.direction === "sell"
        ? "negative"
        : "neutral";
  ...
  // Show notification
  showNotification(
    `Analysis complete: ${overallSignal.recommendation}`,
    overallSignal.direction === "buy"
      ? "success"
      : overallSignal.direction === "sell"
        ? "error"
...
function renderMarketData() {
  ...
  const marketsToShow =
    appState.selectedMarkets.length > 0
...
function updateMarketData() {
  ...
  marketCards.forEach((card) => {
    ...
    const market = appState.brokerConnection.markets.find(
      (m) => m.symbol === symbol,
    );
    ...
    if (market) {
      ...
      valueElement.textContent = market.bid.toFixed(
        market.symbol.includes("BTC") ? 1 : 4,
      );
...
function placeOrder(type) {
  ...
  const result = appState.brokerConnection.placeOrder(
    type,
    appState.activeSymbol,
    amount,
    leverage,
...
function setupEventListeners() {
  ...
  if (notificationBadge) {
    notificationBadge.addEventListener("click", function () {
      ...
      setTimeout(() => {
        showNotification(
          "Chart Analyzer identified potential trading opportunities",
          "info",
        );
      }, 2000);
    });
...
function setupAutoTradingToggle() {
  ...
  const tradingProgress = document
    .getElementById("trading-progress")
    .querySelector(".fill");
    ...
    if (this.checked) {
      ...
      appState.autoTrading = {
        ...
        interval: setInterval(() => {
          ...
          if (Math.random() > 0.8) {
            // 20% chance to place a trade
            const type = Math.random() > 0.5 ? "buy" : "sell";
            const symbol =
              appState.brokerConnection.markets[
                Math.floor(
                  Math.random() * appState.brokerConnection.markets.length,
                )
              ].symbol;
            ...
            if (result.success) {
              // Show notification
              showNotification(
                `Auto Trading: ${type.toUpperCase()} ${symbol} order placed`,
                "success",
              );
              // Update orders table
              updateActiveOrders();
...
// DIRECT BUTTON FIX - Added to solve button functionality issues
function fixAllButtons() {
  console.log("MAGUS PRIME X Button Fix - Initializing...");

  // 1. Fix navigation buttons
  document
    .querySelectorAll(".nav-item, [data-page], .nav-link")
    .forEach((button) => {
  ...
  document
    .querySelectorAll(".analyzer-tab, [data-tab], .tab-button")
    .forEach((tab) => {
      tab.addEventListener("click", function () {
        ...
        document
          .querySelectorAll(".tab-content, .analyzer-content")
          .forEach((content) => {
  ...
  const connectButton = document.querySelector(
    '#connectBtn, .connect-button, [data-action="connect"]',
  );
  if (connectButton) {
    connectButton.addEventListener("click", function () {
      ...
      if (
        window.capitalApi &&
        typeof window.capitalApi.connect === "function"
      ) {
        try {
          window.capitalApi.connect();
  ...
  const startBotButton = document.querySelector(
    '#startBotBtn, .start-bot-button, [data-action="start-bot"]',
  );
  if (startBotButton) {
  ...
  document
    .querySelectorAll(".buy-button, .sell-button, #buyBtn, #sellBtn")
    .forEach((button) => {
      button.addEventListener("click", function () {
        const action =
          this.classList.contains("buy-button") || this.id === "buyBtn"
            ? "buy"
            : "sell";
  ...
  const analyzeButton = document.querySelector(
    '.analyze-button, #analyzeBtn, [data-action="analyze-market"]',
  );
  if (analyzeButton) {
    analyzeButton.addEventListener("click", function () {
      ...
      if (resultsContainer) {
        resultsContainer.innerHTML =
          '<div class="loading">Analyzing market data...</div>';
      }
      ...
      if (window.analyzeMarket && typeof window.analyzeMarket === "function") {
        try {
          ...
          if (
            ...
            typeof window.displayAnalysisResults === "function"
          ) {
  ...
  document
    .querySelectorAll("button:not([data-fixed]), .btn:not([data-fixed])")
    .forEach((button) => {
  ...
  console.log("MAGUS PRIME X Button Fix - Initialized Successfully");
}

// Call the fix function when the document is loaded
document.addEventListener("DOMContentLoaded", function () {
  // Apply button fixes
...
window.addEventListener("load", function () {
  // Apply button fixes again to catch any dynamically created buttons
  setTimeout(fixAllButtons, 1000);
});

// Apply fix when app is fully initialized
window.addEventListener("app-initialized", fixAllButtons);
...
function fixAllButtons() {
  ...
  document
    .querySelectorAll(".nav-item, [data-page], .nav-link, .nav-links a")
    .forEach((button) => {
      button.addEventListener("click", function (e) {
        e.preventDefault();
        const page =
        ...
        if (page) {
          ...
          // Hide all pages
          document
            .querySelectorAll(".page, .page-content, .page-section")
            .forEach((p) => {
              p.style.display = "none";
  ...
  // 3. Fix Connect button for Capital.com integration
  ...
  connectButtons.forEach((button) => {
    button.addEventListener("click", function () {
      const isConnected =
        this.classList.contains("connected") ||
  ...
  // 4. Fix Start Bot button
  const startBotButtons = document.querySelectorAll(
    '.action-button.primary, #startBotBtn, [data-action="start-bot"]',
  );
  startBotButtons.forEach((button) => {
    button.addEventListener("click", function () {
      const isRunning =
        this.classList.contains("running") || this.textContent.includes("Stop");
  ...
  const accountButtons = document.querySelectorAll(
    '#loginBtn, #accountBtn, [data-action="account"]',
  );
  ...
  const currencyButtons = document.querySelectorAll(
    '.dropdown .action-button, [data-action="currency"]',
  );
  ...
  document
    .querySelectorAll(
      '.buy-button, .sell-button, [data-action="buy"], [data-action="sell"]',
    )
    .forEach((button) => {
      button.addEventListener("click", function () {
        const action =
          this.classList.contains("buy-button") ||
          this.getAttribute("data-action") === "buy"
            ? "buy"
            : "sell";
        const symbol =
          this.getAttribute("data-symbol") || window.currentSymbol || "BTCUSD";
  ...
  console.log(
    "MAGUS PRIME X Button Fix - All buttons successfully initialized",
  );
}

// Make sure fixAllButtons is called when the document is ready
document.addEventListener("DOMContentLoaded", function () {
  console.log("MAGUS PRIME X - Document ready, initializing button fixes...");
  fixAllButtons();
});
...
if (
  ...
  console.log(
    "MAGUS PRIME X - Document already loaded, applying button fixes immediately...",
  );
  setTimeout(fixAllButtons, 100); // Small delay to ensure the DOM is fully processed
}
