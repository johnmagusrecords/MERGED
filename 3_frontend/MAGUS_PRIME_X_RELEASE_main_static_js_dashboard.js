...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('clear-history-btn').addEventListener('click', function() {
    if (confirm('Are you sure you want to clear trade history?')) {
      ...
      .then(data => {
        if (data.success) {
          // Update table
          document.getElementById('trade-table-body').innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">No trades found</td></tr>';
  ...
  function refreshData() {
    ...
    .then(([trades, metrics]) => {
      ...
      if (trades.length > 0) {
        tableBody.innerHTML = trades.map((t, idx) => `
        ...
        tableBody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">No trades found</td></tr>';
      }
      ...
      document.getElementById('win-rate').className = parseFloat(winRate) > 50 ? 'text-green-500' : 'text-red-500';
      
      const pnl = calculatePnL(trades);
      document.getElementById('net-pnl').textContent = '$' + pnl;
      document.getElementById('net-pnl').className = parseFloat(pnl) > 0 ? 'text-green-500' : 'text-red-500';
      
      // Update status
      if (metrics) {
        document.getElementById('bot-status').textContent = metrics.paused ? 'Paused' : 'Running';
        document.getElementById('bot-status').className = metrics.paused ? 'text-red-500' : 'text-green-500';
        document.getElementById('strategy-display').textContent = metrics.strategy;
        document.getElementById('interval-display').textContent = metrics.interval || '5 min';
      }
    })
