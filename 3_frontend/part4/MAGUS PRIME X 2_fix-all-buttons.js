...
(function () {
  ...
  function waitForElements() {
    ...
    const allElements = document.querySelectorAll(
      'button, .nav-item, .nav-link, [role="button"], a[href^="#"], .buy-button, .sell-button, #startBotBtn, #connectBtn, #accountBtn, .dropdown-toggle',
    );
    ...
    if (allElements.length > 0) {
      console.log(
        "[Button Fix] Found " +
          allElements.length +
          " interactive elements, applying fixes...",
      );
      fixAllButtons();
  ...
  function fixNavigation() {
    const navButtons = document.querySelectorAll(
      '.nav-item, .nav-link, [data-page], [href^="#"], [data-tab], .tab-link',
    );
    console.log(
      "[Button Fix] Found " + navButtons.length + " navigation buttons",
    );
    ...
    navButtons.forEach((button) => {
      ...
      newButton.addEventListener("click", function (e) {
        ...
        console.log(
          "[Button Fix] Navigation button clicked:",
          this.textContent.trim(),
        );

        // Get target page
        let targetPage =
          this.getAttribute("data-page") || this.getAttribute("data-tab");
        ...
        if (targetPage) {
          ...
          document
            .querySelectorAll(
              ".nav-item, .nav-link, [data-page], [data-tab], .tab-link",
            )
            .forEach((nav) => {
              nav.classList.remove("active", "selected", "current");

              // Also update any child elements that might show active state
              const activeIndicators = nav.querySelectorAll(
                ".active-indicator, .selected-indicator",
              );
          ...
          // Also update any child elements that might show active state
          const activeIndicators = this.querySelectorAll(
            ".active-indicator, .selected-indicator",
          );
          activeIndicators.forEach((indicator) => {
  ...
  function showPage(pageId) {
    ...
    const pages = mainContainer.querySelectorAll(
      '.page-content, .page, [role="tabpanel"], .tab-content, .content-area, .tab-pane, [id$="-content"], [id$="Content"]',
    );
    pages.forEach((page) => {
    ...
    if (!targetFound) {
      const allContainers = document.querySelectorAll(
        '.tab-pane, .page, .content-section, [id$="-content"], [id$="Content"]',
      );
      for (const container of allContainers) {
        // Look for headers or content that match the page ID
        const matchingElements = container.querySelectorAll(
          `h1, h2, h3, h4, .title, .header, .section-header`,
        );
        for (const element of matchingElements) {
          if (
            element.textContent.toLowerCase().includes(pageId.toLowerCase())
          ) {
            container.style.display = "block";
            container.classList.add("active", "show", "visible");
            targetFound = true;
            console.log(
              "[Button Fix] Found and displayed container with matching content:",
              pageId,
            );
            break;
          }
  ...
  function fixActionButtons() {
    ...
    const startBotBtns = Array.from(
      document.querySelectorAll(
        '#startBotBtn, .action-button.primary, button, .button, .btn, [role="button"]',
      ),
    ).filter(
      (el) =>
    ...
    console.log(
      "[Button Fix] Found " + startBotBtns.length + " start bot buttons",
    );
    ...
    const connectBtns = Array.from(
      document.querySelectorAll(
        '#connectBtn, .connect-button, button, .button, .btn, [role="button"]',
      ),
    ).filter(
      (el) =>
        (el.textContent.trim().includes("Connect") &&
          !el.textContent.trim().includes("Connected")) ||
        el.id === "connectBtn" ||
        el.classList.contains("connect-button"),
    );

    console.log(
      "[Button Fix] Found " + connectBtns.length + " connect buttons",
    );
      ...
      newButton.addEventListener("click", function (e) {
        ...
        if (!isConnected) {
          ...
          if (typeof window.connectToBroker === "function") {
            ...
            console.log(
              "[Button Fix] No connect function found, trying to find login modal",
            );
    ...
    const tradeButtons = Array.from(
      document.querySelectorAll(
        ".buy-button, .sell-button, button, .button, .btn",
      ),
    ).filter(
      (el) =>
