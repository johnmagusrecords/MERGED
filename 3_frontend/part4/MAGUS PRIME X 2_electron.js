...
function startPythonBackend() {
  ...
  pythonProcess = spawn("python", ["src/trading_bot.py"], {
...
// Capital.com API credentials - CLEAN VERSION FOR DISTRIBUTION
ipcMain.on("save-credentials", (event, credentials) => {
  // Clear any previously stored credentials first
...
ipcMain.on("get-credentials", (event) => {
  // For the distribution version, don't load any stored credentials
  // This ensures that each new user has to enter their own information
  const storedCredentials = store.get("capital-credentials");

  // Only for distribution - always return empty credentials to force users to enter their own
  event.reply("credentials", null);

  // Also clear any stored credentials from previous installs
  store.delete("capital-credentials");
});
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("magusprimex", process.execPath, [
