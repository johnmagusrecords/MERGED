...
def load_trade_history():
    ...
    if os.path.exists("trade_history.csv"):
        ...
        return df
    return pd.DataFrame(
        columns=["Time", "Symbol", "Action", "Entry", "TP", "SL", "Result", "PnL"]
    )
...
https://cdn.jsdelivr.net/npm/bootstrap@5 + ".2.3/dist/css/bootstrap.min.css" rel="stylesheet">
        <link href=" "
https://fonts.googleapis.com/css2?family + "=Roboto:wght@300;400;500;700&display=swa + "p" rel="stylesheet">
        <link rel="stylesheet" href=" "
https://cdn.jsdelivr.net/npm/bootstrap-i + "cons@1.10.0/font/bootstrap-icons.css">
                ...
                marketData.forEach(item => {
                    ...
                    marketPanel.innerHTML += `
                        <div class="market-item">
                            ...
                            <div class=" "
market-change ${isPositive ? 'positive'  + ": 'negative'}">${item.change}</div>
                        </div>
                    `;
                });
                            ...
                            if (data.success) {
                                ...
                                document.querySelectorAll('.strategy-btn').forEach(btn => {
                                    btn.classList.remove(
                                                            'btn-success',
                                                            'btn-primary',
                                                            'btn-danger');                                    btn.classList.add('btn-outline-secondary');
                                });
...
<tr><td colspan="4" class="text-center p + 'y-4 text-muted">No trades found</td></tr + '>';
                        ...
                        if (trades.length > 0) {
                            tableBody.innerHTML = trades.slice(0, 10).map((t, idx) => `
                                <tr>
                                    ...
                                    <td>${t.Symbol}</td>
                                    <td class=" "
${t.Action === 'BUY' ? 'text-success' :  + "'text-danger'}">${t.Action}</td>
                                    <td>
                                        ${t.Result === 'WIN' 
...
<tr><td colspan="4" class="text-center p + 'y-4 text-muted">No trades found</td></tr + '>';
                        ...
                        if (metrics) {
                            document.getElementById(
                                                       'account-balance').textContent = '$' + (metrics.account_balance || '10,
                                                       000.00');                            document.getElementById('active-positions').textContent = metrics.active_positions || '0';
    ...
    try:
        ...
        if os.path.exists("trade_history.csv"):
            ...
            send_telegram_message(
                f"🗑️ Trade history cleared. Backup saved as {backup_path}"
            )
...
if __name__ == "__main__":
    try:
        ...
        send_telegram_message(
            "🚀 MAGUS PRIME X Bot Dashboard Started\n"
