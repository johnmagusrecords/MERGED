    ...
    def get_performance_metrics(self, trade_history_file):
        ...
        try:
            ...
            trades["PnL"] = trades.apply(
                lambda x: float(x["TP"]) if x["Result"] == "WIN" else -float(x["SL"]),
                axis=1,
            )
            net_profit = trades["PnL"].sum()

            # Calculate Sharpe ratio (assuming daily returns)
            daily_returns = trades.groupby(pd.to_datetime(trades["Time"]).dt.date)[
                "PnL"
            ].sum()
            sharpe_ratio = (
    ...
    def get_strategy_performance(self, trades):
        ...
        try:
            ...
            def calc_strategy_metrics(strategy_trades):
                ...
                return {"winRate": round(win_rate, 2), "profit": round(profit, 2)}
