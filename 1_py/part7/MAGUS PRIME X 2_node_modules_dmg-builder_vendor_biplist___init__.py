...
offsetSize,
                             ...
                             offsetTableOffset')PlistByteCounts = namedtuple(
                                'PlistByteCounts',
                                ' '
nullBytes,
                                boolBytes,
...
Too many offsets to represent in size of + " object reference representation.")
            ...
            if self.trailer.topLevelObjectNumber >= self.trailer.offsetCount:
                raise InvalidPlistException(" "
Top level object number is larger than t + "he number of objects.")
            ...
            while offset_i < self.trailer.offsetCount:
                ...
                if end > offset_table_length:
                    raise InvalidPlistException(
                                                   " "
End of object is at invalid offset %d in + " offset table of length %d" % (end,
                                                   offset_table_length))                tmp_contents = offset_contents[begin:end]
    ...
    def setCurrentOffsetToObjectNumber(self, objectNumber):
        ...
        if self.currentOffset in self.offsetsStack:
            raise InvalidPlistException(" "
Recursive data structure detected in obj + "ect: %d" % objectNumber)
    ...
    def readObject(self):
        ...
        # dict
            ...
            raise InvalidPlistException(
                                           " "
Invalid object found: {format: %s,
                                           extra + ": %s}" % (bin(format),
                                           bin(extra)))        self.endOffsetProtection(protection)
        return result
    ...
    def getSizedInteger(self, data, byteSize, as_number=False):
        """ "
Numbers of 8 bytes are signed integers w + "hen they refer to numbers, but unsigned  + "otherwise."""
        result = 0
        if byteSize == 0:
...
class PlistWriter(object):
    ...
    def computeOffsets(self, obj, asReference=False, isRoot=False):
        ...
        if obj is None:
            ...
            raise InvalidPlistException(
                                           "Unknown object type: %s (%s)" % (type(obj).__name__,
                                           repr(obj)))
    def writeObjectReference(self, obj, output):
        ...
        if position is None:
            ...
            output += self.binaryInt(
                                        len(self.writtenReferences) - 1,
                                        byteSize=self.trailer.objectRefSize)            return (True, output)
        else:
    ...
    def writeOffsetTable(self, output):
        ...
        for obj,order in writtenReferences:
            ...
            if position is None:
                raise InvalidPlistException(" "
Error while writing offsets table. Objec + "t not found. %s" % obj)
    ...
    def binaryInt(self, obj, byteSize=None, as_number=False):
        ...
        if byteSize == 1:
            ...
            raise InvalidPlistException(" "
Core Foundation can't handle integers wi + "th size greater than 16 bytes.")
        return result
    ...
    def intSize(self, obj):
        ...
        # 0x7FFFFFFFFFFFFFFF is the max.
            ...
            raise InvalidPlistException(" "
Core Foundation can't handle integers wi + "th size greater than 8 bytes.")
