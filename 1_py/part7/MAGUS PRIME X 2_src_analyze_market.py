...
async def analyze_market(symbol: str) -> tuple[str, float, str]:
    ...
    try:
        ...
        if last_rsi < TA_PARAMS["RSI_OVERSOLD"] and last_macd > last_signal:
            logging.info(
                f"[SIGNAL] Buy signal generated for {symbol} at price: {last_close}, mode: {mode}"
            )
            return "BUY", last_close, mode
        elif last_rsi > TA_PARAMS["RSI_OVERBOUGHT"] and last_macd < last_signal:
            logging.info(
                f"[SIGNAL] Sell signal generated for {symbol} at price: {last_close}, mode: {mode}"
            )
            return "SELL", last_close, mode

        logging.info(
            f"[INFO] No trading signal for {symbol}. Last close: {last_close}, mode: {mode}"
        )
        return "HOLD", last_close, mode

    except Exception as e:
        logging.error(
            f"[EXCEPTION] Error during market analysis for {symbol}: {str(e)}"
        )
