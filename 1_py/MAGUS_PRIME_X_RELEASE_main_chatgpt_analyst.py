...
class ChatGPTAnalyst:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            self.logger.warning("OpenAI API key not found. ChatGPT integration will not function.")
        else:
    ...
    def analyze_market(self, symbol: str, timeframe: str, data: pd.DataFrame, 
                       news_items: List[Dict], indicators: Dict) -> Dict:
        """
        ...
        try:
            ...
            if cache_key in self.cache:
                ...
                if datetime.now() - cache_item['timestamp'] < timedelta(seconds=self.cache_duration):
            ...
            market_context = self._prepare_market_context(symbol, timeframe, data, indicators)
            
            # Prepare news summary
            news_summary = self._prepare_news_summary(news_items)
            
            # Construct the prompt
            prompt = self._construct_analysis_prompt(symbol, timeframe, market_context, news_summary)
    ...
    def analyze_trading_opportunity(self, symbol: str, timeframe: str, 
                                    ...
                                    data: pd.DataFrame, indicators: Dict) -> Dict:
        """
...
"""
    ...
    def generate_trade_script(self, symbol: str, timeframe: str, data: pd.DataFrame, 
                              indicators: Dict, strategy_type: str) -> str:
        """
...
Technical Indicators:
{json.dumps({k: str(v[-5:]) if isinstance(v, np.ndarray) else v for k, v in indicators.items()}, indent=2)}
...
"""
    ...
    def _prepare_market_context(self, symbol: str, timeframe: str, 
                               data: pd.DataFrame, indicators: Dict) -> Dict:
        ...
        try:
            ...
            sma50 = np.mean(close_prices[-50:]) if len(close_prices) >= 50 else np.mean(close_prices)
            ...
            context = {
                ...
                "daily_change_percent": ((close_prices[-1] / close_prices[-2]) - 1) * 100 if len(close_prices) > 1 else 0,
                "weekly_change_percent": ((close_prices[-1] / close_prices[-7]) - 1) * 100 if len(close_prices) > 6 else 0,
                "volatility_percent": volatility,
                "trend": trend,
                "key_indicators": processed_indicators,
                "recent_prices": recent_data[['open', 'high', 'low', 'close']].to_dict(orient='records')
            }
    ...
    def _prepare_news_summary(self, news_items: List[Dict]) -> str:
        ...
        for i, item in enumerate(news_items[:5]):  # Limit to 5 most recent news items
            ...
            summary += f"   Sentiment: {item.get('sentiment', 'unknown')}, Impact: {item.get('impact', 'unknown')}\n"
            summary += f"   {item.get('description', 'No description available.')[:100]}...\n\n"
            
        return summary

    def _construct_analysis_prompt(self, symbol: str, timeframe: str, 
                                 market_context: Dict, news_summary: str) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                response = openai.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are a financial market analysis expert with deep experience in technical analysis, trading strategies, and market psychology."},
                ...
                self.logger.warning(f"ChatGPT API error (attempt {attempt+1}/{self.max_retries}): {str(e)}")
...
class DeepSeekerAnalyst:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            self.logger.warning("DeepSeeker API key not found. DeepSeeker integration will not function.")
        self.cache = {}
    ...
    def analyze_market(self, symbol: str, timeframe: str, data: pd.DataFrame, 
        ...
        return {
            "sentiment": "bullish" if data['close'].iloc[-1] > data['close'].iloc[-2] else "bearish",
            "confidence": 0.75,
            "recommendation": f"Consider {('long' if data['close'].iloc[-1] > data['close'].iloc[-2] else 'short')} positions in {symbol}",
...
class AIMarketAnalyst:
    ...
    def get_comprehensive_analysis(self, symbol: str, timeframe: str, data: pd.DataFrame, 
                                  news_items: List[Dict], indicators: Dict) -> Dict:
        """Get comprehensive analysis from multiple AI sources"""
        try:
            # Collect analyses from different sources
            chatgpt_analysis = self.chatgpt.analyze_market(symbol, timeframe, data, news_items, indicators)
            deepseeker_analysis = self.deepseeker.analyze_market(symbol, timeframe, data, news_items, indicators)
            ...
            aggregated = {
                ...
                "consensus": {
                    "sentiment": self._get_consensus_sentiment(chatgpt_analysis, deepseeker_analysis),
                    "confidence": self._get_consensus_confidence(chatgpt_analysis, deepseeker_analysis, chart_patterns),
                    "trading_bias": self._get_consensus_bias(chatgpt_analysis, deepseeker_analysis, chart_patterns)
                }
            }
    ...
    def _get_consensus_sentiment(self, chatgpt_analysis: Dict, deepseeker_analysis: Dict) -> str:
        ...
        return chatgpt_sentiment if chatgpt_confidence > deepseeker_confidence else deepseeker_sentiment
        
    def _get_consensus_confidence(self, chatgpt_analysis: Dict, deepseeker_analysis: Dict, chart_patterns: Dict) -> float:
    ...
    def _get_consensus_bias(self, chatgpt_analysis: Dict, deepseeker_analysis: Dict, chart_patterns: Dict) -> str:
        ...
        deepseeker_sentiment = deepseeker_analysis.get("sentiment", "neutral")
        deepseeker_bias = "long" if deepseeker_sentiment == "bullish" else "short" if deepseeker_sentiment == "bearish" else "neutral"
        biases.append((deepseeker_bias, deepseeker_analysis.get("confidence", 0.5)))
        
        # Chart patterns bias
        chart_bias = chart_patterns.get("overall_bias", "neutral")
        chart_bias_mapped = "long" if chart_bias == "bullish" else "short" if chart_bias == "bearish" else "neutral"
