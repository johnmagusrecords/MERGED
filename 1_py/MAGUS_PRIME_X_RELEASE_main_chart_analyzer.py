...
class ChartAnalyzer:
    ...
    def analyze_candles(self, symbol: str, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(patterns_found) == 0:
                ...
                sentiment_strength = abs(bullish_count - bearish_count) / len(patterns_found)
            ...
            self.logger.error(f"Error analyzing candlestick patterns for {symbol}: {str(e)}")
            return {
                "symbol": symbol,
                "error": str(e),
                "patterns": [],
    ...
    def _get_pattern_description(self, pattern_name: str) -> str:
        ...
        descriptions = {
            ...
            "Hammer": "Bullish reversal pattern with a small body and long lower shadow, indicating potential trend reversal.",
            "Shooting Star": "Bearish reversal pattern with a small body and long upper shadow, signaling potential downtrend.",
            "Engulfing Pattern": "A two-candle pattern where the second candle completely engulfs the body of the first.",
            "Morning Star": "Bullish reversal pattern consisting of three candles, indicating a potential uptrend.",
            "Evening Star": "Bearish reversal pattern consisting of three candles, indicating a potential downtrend.",
            "Three Black Crows": "Bearish reversal pattern with three consecutive declining candles, suggesting a downtrend.",
            "Three White Soldiers": "Bullish reversal pattern with three consecutive advancing candles, suggesting an uptrend."
        }
        
        return descriptions.get(pattern_name, f"A {pattern_name.lower()} pattern was detected.")
            
    def analyze_trend(self, symbol: str, data: pd.DataFrame, 
        ...
        try:
            ...
            for tf, tf_data in data.items():
                ...
                adx = talib.ADX(tf_data['high'].values, tf_data['low'].values, close_prices, timeperiod=14)
                ...
                if (current_price > current_ema20 > current_sma20 > current_sma50 > current_sma200):
                ...
                elif (current_price < current_ema20 < current_sma20 < current_sma50 < current_sma200):
                ...
                trend_analysis["timeframes"][tf] = {
                    ...
                    "sma20_cross": True if (close_prices[-2] - sma20[-2]) * (current_price - current_sma20) < 0 else False
                }
            ...
            if trend_analysis["timeframes"]:
                ...
                if total_weight > 0:
                    if uptrend_score > downtrend_score:
                        ...
                        trend_analysis["overall_strength"] = uptrend_score / total_weight
                    elif downtrend_score > uptrend_score:
                        trend_analysis["overall_trend"] = "downtrend"
                        trend_analysis["overall_strength"] = downtrend_score / total_weight
                    else:
    ...
    def _identify_support_resistance(self, data: pd.DataFrame, window_size: int = 10, threshold: float = 0.01) -> List[Support]:
        """
        ...
        try:
            ...
            if len(levels) < 4:
                ...
                return self._identify_support_resistance(data, window_size=5, threshold=threshold)
            ...
            for i in range(1, len(levels)):
                ...
                group_avg = sum(level.price for level in current_group) / len(current_group)
            ...
            for i, level in enumerate(grouped_levels):
                ...
                for j in range(len(data)):
                    ...
                    if abs(data['high'].iloc[j] - level.price) <= touch_threshold or \
                       abs(data['low'].iloc[j] - level.price) <= touch_threshold:
                        touches += 1
                ...
                latest_touch = max(level.time_period) if level.time_period else data.index[0]
                days_since = (data.index[-1] - latest_touch).days if hasattr(latest_touch, 'days') else 0
                recency_score = max(0.0, 1.0 - (days_since / 30))  # Scale to 0-1, 0 means 30+ days old
                ...
                strength = (recency_weight * recency_score) + (touch_weight * touch_score)
    ...
    def _generate_pattern_chart(self, data: pd.DataFrame, patterns: List[CandlePattern], 
                               support_resistance: List[Support], chart_patterns: List[Dict] = None,
        ...
        try:
            ...
            if 'date' not in plot_data.columns and not isinstance(plot_data.index, pd.DatetimeIndex):
                # Create a date index if not present
                plot_data = plot_data.set_index(pd.date_range(start='2023-01-01', periods=len(plot_data)))
            ...
            for pattern in patterns:
                ...
                min_price = plot_data.loc[plot_data.index[start_idx:end_idx+1], 'low'].min() * 0.99
                max_price = plot_data.loc[plot_data.index[start_idx:end_idx+1], 'high'].max() * 1.01
                ...
                for pattern in chart_patterns:
                    if 'position' in pattern and len(pattern['position']) >= 2:
                        ...
                        if 'Double Top' in pattern_name:
                            ...
                            rect = patches.Rectangle(
                                (plot_data.index[start], plot_data.loc[plot_data.index[start:end], 'low'].min() * 0.99), 
                                plot_data.index[end] - plot_data.index[start], 
                                peak1_y * 1.01 - plot_data.loc[plot_data.index[start:end], 'low'].min() * 0.99, 
                                linewidth=1, 
                                edgecolor=color, 
                                facecolor=color, 
                                alpha=0.1
                            ...
                            rect = patches.Rectangle(
                                ...
                                plot_data.index[end] - plot_data.index[start], 
                                plot_data.loc[plot_data.index[start:end], 'high'].max() * 1.01 - trough1_y * 0.99, 
                                linewidth=1, 
                                edgecolor=color, 
                                facecolor=color, 
                                alpha=0.1
                            ...
                            if 'Inverse' in pattern_name:
                                ...
                                # Get neckline
                                left_peak = plot_data.loc[plot_data.index[left:head], 'high'].max()
                                right_peak = plot_data.loc[plot_data.index[head:right], 'high'].max()
                                ...
                                # Get neckline
                                left_trough = plot_data.loc[plot_data.index[left:head], 'low'].min()
                                right_trough = plot_data.loc[plot_data.index[head:right], 'low'].min()
                            ...
                            ax.plot(
                                [plot_data.index[left], plot_data.index[head], plot_data.index[right]],
                            ...
                            if 'Ascending' in pattern_name:
                                ...
                                resistance = plot_data.loc[plot_data.index[start:end], 'high'].max()
                                ax.axhline(y=resistance, xmin=start/len(plot_data), xmax=end/len(plot_data),
                                ...
                                support = plot_data.loc[plot_data.index[start:end], 'low'].min()
                                ax.axhline(y=support, xmin=start/len(plot_data), xmax=end/len(plot_data),
                                          color=color, linestyle='--', linewidth=2)
                                
                                # Plot descending resistance line
                                first_high = plot_data.loc[plot_data.index[start], 'high']
                            ...
                            range_highs = plot_data.loc[plot_data.index[start:end], 'high']
                            range_lows = plot_data.loc[plot_data.index[start:end], 'low']
                            
                            # Fit lines
                            x = np.arange(start, end+1)
                            x_dates = [plot_data.index[i] for i in range(start, end+1)]
                            
                            # Calculate slopes and intercepts
                            high_slope, high_intercept = np.polyfit(x, range_highs.values, 1)
                            low_slope, low_intercept = np.polyfit(x, range_lows.values, 1)
                            
                            # Plot channel lines
                            upper_line = high_intercept + high_slope * x
                            lower_line = low_intercept + low_slope * x
                            
                            ax.plot(x_dates, upper_line, color=color, linewidth=2, linestyle='-')
                            ax.plot(x_dates, lower_line, color=color, linewidth=2, linestyle='-')
                            
                            # Add shaded region
                            ax.fill_between(x_dates, upper_line, lower_line, color=color, alpha=0.1)
            
            # Set plot title and labels
            symbol = data.get('symbol', plot_data.columns.name if hasattr(plot_data.columns, 'name') else 'Price')
            ...
            for name, props in pattern_annotations.items():
                handle = patches.Patch(color=props['color'], alpha=0.3, label=f"{name} ({'Bullish' if props['bullish'] else 'Bearish'})")
    ...
    def detect_chart_patterns(self, symbol: str, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            head_shoulders = self._detect_head_and_shoulders(high_prices, low_prices, close_prices)
            if head_shoulders:
                patterns_found.append(head_shoulders)
                
            # Detect triangles
            ascending_triangle = self._detect_ascending_triangle(high_prices, low_prices)
            if ascending_triangle:
                patterns_found.append(ascending_triangle)
                
            descending_triangle = self._detect_descending_triangle(high_prices, low_prices)
            ...
            if patterns_found:
                bullish_patterns = sum(1 for p in patterns_found if p.get('bias') == 'bullish')
                bearish_patterns = sum(1 for p in patterns_found if p.get('bias') == 'bearish')
    ...
    def _detect_double_top(self, high_prices, close_prices, threshold=0.03, min_gap=5) -> Optional[Dict]:
            ...
            for i in range(min_gap, n - min_gap):
                ...
                if (high_prices[i] > high_prices[i-1] and high_prices[i] > high_prices[i+1]):
                    ...
                    for j in range(first_peak + min_gap, n - 1):
                        if (high_prices[j] > high_prices[j-1] and high_prices[j] > high_prices[j+1]):
                            second_peak = j
                            second_peak_value = high_prices[j]
                            
                            # Check if peaks are at similar heights
                            price_diff = abs(first_peak_value - second_peak_value) / first_peak_value
                            if price_diff < threshold:
                                # Check for confirmation (price breaking below the trough)
                                trough = min(close_prices[first_peak:second_peak])
                                trough_idx = first_peak + np.argmin(close_prices[first_peak:second_peak])
                                
                                if second_peak + 1 < n and close_prices[second_peak+1] < trough:
    ...
    def _detect_double_bottom(self, low_prices, close_prices, threshold=0.03, min_gap=5) -> Optional[Dict]:
            ...
            for i in range(min_gap, n - min_gap):
                ...
                if (low_prices[i] < low_prices[i-1] and low_prices[i] < low_prices[i+1]):
                    ...
                    for j in range(first_trough + min_gap, n - 1):
                        if (low_prices[j] < low_prices[j-1] and low_prices[j] < low_prices[j+1]):
                            second_trough = j
                            second_trough_value = low_prices[j]
                            
                            # Check if troughs are at similar heights
                            price_diff = abs(first_trough_value - second_trough_value) / first_trough_value
                            if price_diff < threshold:
                                # Check for confirmation (price breaking above the peak)
                                peak = max(close_prices[first_trough:second_trough])
                                peak_idx = first_trough + np.argmax(close_prices[first_trough:second_trough])
                                
                                if second_trough + 1 < n and close_prices[second_trough+1] > peak:
                                    confidence = 0.7 * (1 - price_diff/threshold)
    ...
    def _detect_head_and_shoulders(self, high_prices, low_prices, close_prices, threshold=0.03, min_gap=3) -> Optional[Dict]:
                        ...
                        if high_prices[j] > high_prices[j-1] and high_prices[j] > high_prices[j+1]:
                            ...
                            if head_value > left_shoulder_value:
                                ...
                                for k in range(head + min_gap, n - 1):
                                    if high_prices[k] > high_prices[k-1] and high_prices[k] > high_prices[k+1]:
                                        right_shoulder = k
                                        right_shoulder_value = high_prices[k]
                                        
                                        # Right shoulder should be similar height to left shoulder
                                        shoulder_diff = abs(left_shoulder_value - right_shoulder_value) / left_shoulder_value
                                        
                                        if shoulder_diff < threshold and right_shoulder_value < head_value:
                                            # Find neckline
                                            left_trough = min(low_prices[left_shoulder:head])
                                            right_trough = min(low_prices[head:right_shoulder])
                                            neckline = (left_trough + right_trough) / 2
                                            
                                            # Check for confirmation
                                            if right_shoulder + 1 < n and close_prices[right_shoulder+1] < neckline:
                                                confidence = 0.8 * (1 - shoulder_diff/threshold)
                                                
                                                return {
                                                    "name": "Head and Shoulders",
                                                    "bias": "bearish",
                                                    "position": [left_shoulder, head, right_shoulder],
                                                    "confidence": confidence,
                                                    "target": neckline - (head_value - neckline)
                                                }
                    ...
                    for j in range(left_shoulder + min_gap, n - min_gap):
                        if low_prices[j] < low_prices[j-1] and low_prices[j] < low_prices[j+1]:
                            ...
                            if head_value < left_shoulder_value:
                                # Look for right shoulder
                                for k in range(head + min_gap, n - 1):
                                    if low_prices[k] < low_prices[k-1] and low_prices[k] < low_prices[k+1]:
                                        right_shoulder = k
                                        ...
                                        if shoulder_diff < threshold and right_shoulder_value > head_value:
                                            # Find neckline
                                            left_peak = max(high_prices[left_shoulder:head])
                                            right_peak = max(high_prices[head:right_shoulder])
                                            neckline = (left_peak + right_peak) / 2
                                            ...
                                            if right_shoulder + 1 < n and close_prices[right_shoulder+1] > neckline:
                                                ...
                                                return {
                                                    ...
                                                    "position": [left_shoulder, head, right_shoulder],
                                                    "confidence": confidence, 
                                                    "target": neckline + (neckline - head_value)
                                                }
            return None
    ...
    def _detect_ascending_triangle(self, high_prices, low_prices, min_touches=3, threshold=0.03) -> Optional[Dict]:
            ...
            for i in range(n - min_touches * 5):
                ...
                if high_prices[i] > high_prices[i-1] and high_prices[i] > high_prices[i+1]:
                    ...
                    for j in range(i, n):
                        ...
                        if abs(high_prices[j] - resistance_level) / resistance_level < threshold:
                            touches += 1
                            
                        # Track lows for trend line
                        if j > i and low_prices[j] < low_prices[j-1] and low_prices[j] < low_prices[j+1] if j+1 < n else True:
    ...
    def _detect_descending_triangle(self, high_prices, low_prices, min_touches=3, threshold=0.03) -> Optional[Dict]:
            ...
            for i in range(n - min_touches * 5):
                ...
                if low_prices[i] < low_prices[i-1] and low_prices[i] < low_prices[i+1]:
                    ...
                    for j in range(i, n):
                        ...
                        if abs(low_prices[j] - support_level) / support_level < threshold:
                            touches += 1
                            
                        # Track highs for trend line
                        if j > i and high_prices[j] > high_prices[j-1] and high_prices[j] > high_prices[j+1] if j+1 < n else True:
    ...
    def _detect_channel(self, high_prices, low_prices, close_prices, min_touches=3, threshold=0.05) -> Optional[Dict]:
        ...
        try:
            ...
            for start in range(0, n - window, window // 2):
                ...
                if abs(high_slope - low_slope) > 0.001 * abs(high_slope + low_slope) / 2:
                    continue
                ...
                if upper_touches >= min_touches and lower_touches >= min_touches:
                    ...
                    if abs(last_close - upper_line_end) < abs(last_close - lower_line_end):
    ...
    def comprehensive_analysis(self, symbol: str, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            self._generate_pattern_chart(data, pattern_analysis['patterns'], support_resistance, chart_patterns['patterns'], filename)
            ...
            self.logger.error(f"Error performing comprehensive analysis for {symbol}: {str(e)}")
            return {
                "symbol": symbol,
                "error": str(e)
            }

    def calculate_technical_indicators(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            indicators['tenkan_sen'] = self._calculate_ichimoku_tenkan_sen(high_prices, low_prices)
            indicators['kijun_sen'] = self._calculate_ichimoku_kijun_sen(high_prices, low_prices)
            indicators['senkou_span_a'] = self._calculate_ichimoku_senkou_span_a(indicators['tenkan_sen'], indicators['kijun_sen'])
            indicators['senkou_span_b'] = self._calculate_ichimoku_senkou_span_b(high_prices, low_prices)
            indicators['chikou_span'] = np.append(np.array([np.nan] * 26), close_prices[:-26])
    ...
    def analyze_market(self, symbol: str, data: pd.DataFrame, timeframe: str = "daily", save_chart: bool = True) -> Dict:
        """
        ...
        """
        try:
            self.logger.info(f"Performing comprehensive market analysis for {symbol} on {timeframe} timeframe")
            ...
            chart_path = None
            if save_chart:
                filename = os.path.join(self.chart_images_dir, f"{symbol.replace('/', '_')}_{timeframe}_chart.png")
                os.makedirs(os.path.dirname(filename), exist_ok=True)
            ...
            if (indicators.get('sma_20', np.array([]))[-1] > indicators.get('sma_50', np.array([]))[-1] and 
                indicators.get('sma_20', np.array([]))[-2] <= indicators.get('sma_50', np.array([]))[-2]):
                bullish_signals += 1
            elif (indicators.get('sma_20', np.array([]))[-1] < indicators.get('sma_50', np.array([]))[-1] and 
                  indicators.get('sma_20', np.array([]))[-2] >= indicators.get('sma_50', np.array([]))[-2]):
                bearish_signals += 1
            ...
            for pattern in pattern_analysis.get('patterns', []):
                if pattern.get('name') in ["Hammer", "Morning Star", "Bullish Engulfing"] and pattern.get('strength', 0) > 0.7:
                    trading_signal = "buy"
                    break
                elif pattern.get('name') in ["Shooting Star", "Evening Star", "Bearish Engulfing"] and pattern.get('strength', 0) > 0.7:
                    trading_signal = "sell"
                    break
            ...
            result = {
                ...
                "technical_indicators": {
                    ...
                    "macd": {
                        ...
                        "signal": float(last_macd_signal) if not np.isnan(last_macd_signal) else None,
                        "histogram": float(indicators.get('macd_hist', np.array([]))[-1]) if len(indicators.get('macd_hist', np.array([]))) > 0 and not np.isnan(indicators.get('macd_hist', np.array([]))[-1]) else None
                    },
                    "moving_averages": {
                        "sma_20": float(indicators.get('sma_20', np.array([]))[-1]) if len(indicators.get('sma_20', np.array([]))) > 0 and not np.isnan(indicators.get('sma_20', np.array([]))[-1]) else None,
                        "sma_50": float(indicators.get('sma_50', np.array([]))[-1]) if len(indicators.get('sma_50', np.array([]))) > 0 and not np.isnan(indicators.get('sma_50', np.array([]))[-1]) else None,
                        "sma_200": float(indicators.get('sma_200', np.array([]))[-1]) if len(indicators.get('sma_200', np.array([]))) > 0 and not np.isnan(indicators.get('sma_200', np.array([]))[-1]) else None
                    }
                },
                "market_summary": {
                    "overall_bias": overall_bias,
                    ...
                    "key_levels": {
                        "nearest_support": min([level.price for level in support_resistance if level.type == 'support'] or [0], key=lambda x: abs(x - data['close'].iloc[-1])) if len(data) > 0 else None,
                        "nearest_resistance": min([level.price for level in support_resistance if level.type == 'resistance'] or [float('inf')], key=lambda x: abs(x - data['close'].iloc[-1])) if len(data) > 0 else None
                    }
                }
            }
            ...
            self.logger.error(f"Error in comprehensive market analysis for {symbol}: {str(e)}")
            return {
                "symbol": symbol,
                "timestamp": datetime.now().isoformat(),
                "error": str(e),
