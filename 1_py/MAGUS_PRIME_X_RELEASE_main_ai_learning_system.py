    ...
    def step(self, action):
        ...
        if self.current_step >= self.max_steps - 1:
            ...
            if self.position != 0:
                ...
                if self.position == 1:
                    profit_pct = (final_price - self.position_price) / self.position_price
                else:
                    profit_pct = (self.position_price - final_price) / self.position_price
                self.balance *= (1 + profit_pct)
...
class TradeEvaluationCallback(BaseCallback):
    ...
    def _on_step(self):
        if self.n_calls % 1000 == 0:
            ...
            logger.info(f"Evaluation at step {self.n_calls}: Total return: {total_reward:.4f}")
...
class AILearningSystem:
            ...
            try:
                with open(self.decision_db_path, 'r') as f:
                    ...
                    for decision in self.decisions_history:
                        decision['timestamp'] = datetime.datetime.fromisoformat(decision['timestamp'])
                self.logger.info(f"Loaded {len(self.decisions_history)} historical decisions")
    ...
    def record_trade_result(self, trade_id, result, profit_loss, lessons_learned=None):
        ...
        for decision in self.decisions_history:
            if (decision.get('symbol') == trade_id.get('symbol') and 
                decision.get('timestamp') == trade_id.get('timestamp')):
                ...
                try:
                    ...
                    self.logger.error(f"Error saving updated decision history: {str(e)}")
    ...
    def _identify_successful_patterns(self, decision, market_data):
        ...
        if ('trend_direction' in decision['market_context'] and 
            decision['action'] == 'buy' and decision['market_context']['trend_direction'] > 0):
            patterns.append("Long position aligned with uptrend")
            
        elif ('trend_direction' in decision['market_context'] and 
              decision['action'] == 'sell' and decision['market_context']['trend_direction'] < 0):
    ...
    def _identify_mistakes(self, decision, market_data):
        ...
        # Check entry timing
        if (abs(decision['entry_price'] - decision['stop_loss']) > 
    ...
    def train_reinforcement_model(self, symbol, market_data):
        ...
        try:
            ...
            # Save the scaler
            scaler_path = os.path.join(self.model_dir, f"{symbol.lower()}_feature_scaler.pkl")
    ...
    def analyze_historical_performance(self):
        ...
        for decision in self.decisions_history:
            ...
            for indicator, value in decision.get('indicators_used', {}).items():
    ...
    def generate_trading_insights(self):
        ...
        if analysis['overall_win_rate'] > 0.6:
            insights.append(f"System shows strong performance with {analysis['overall_win_rate']:.2%} win rate")
        elif analysis['overall_win_rate'] < 0.4:
            insights.append(f"System needs improvement with only {analysis['overall_win_rate']:.2%} win rate")
            
        # Profit factor insight
        if analysis['profit_factor'] > 2:
            insights.append(f"Excellent profit factor of {analysis['profit_factor']:.2f}. Keep position sizing consistent.")
        elif analysis['profit_factor'] < 1:
            insights.append(f"Warning: Profit factor below 1 ({analysis['profit_factor']:.2f}). Losers are bigger than winners.")
            
        # Best performing symbols
        best_symbols = sorted(
            [(s, data) for s, data in analysis['symbol_performance'].items() if data['trades'] >= 5],
        ...
        if best_symbols:
            ...
            insights.append(f"Best performing asset: {top_symbol} with {top_data['win_rate']:.2%} win rate over {top_data['trades']} trades")
        ...
        if worst_symbols:
            ...
            insights.append(f"Worst performing asset: {bottom_symbol} with only {bottom_data['win_rate']:.2%} win rate")
            
        # Best time to trade
        best_hours = sorted(
            [(h, data) for h, data in analysis['hour_performance'].items() if data['trades'] >= 5],
            key=lambda x: x[1]['win_rate'],
            reverse=True
        )
        ...
        if best_hours:
            ...
            insights.append(f"Best trading hour: {top_hour}:00 with {top_hour_data['win_rate']:.2%} win rate")
            
        # Best indicators
        best_indicators = sorted(
            [(i, data) for i, data in analysis['indicator_performance'].items() 
        ...
        if best_indicators:
            ...
            insights.append(f"Most reliable signal: {top_indicator} with {top_ind_data.get('win_rate', 0):.2%} win rate")
