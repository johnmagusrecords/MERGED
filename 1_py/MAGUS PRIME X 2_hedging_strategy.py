...
class HedgingStrategy:
    ...
    def check_hedge_conditions(
        self, position: Position, market_data: Dict, indicators: Dict
        ...
        try:
            ...
            divergence = self._detect_divergence(
                position, data_5m, indicators[symbol]["5m"]
            )
            if divergence:
    ...
    def _detect_trend_reversal(
        ...
        try:
            ...
            if position_direction == "long":
                bearish = (
                    current_price < ema_200_5m
                    and current_price < ema_200_1h
                    and ind_5m["rsi"][-1] < 30
                    and ind_1h["macd_hist"][-1] < 0
                )
                return bearish
            else:
                bullish = (
    ...
    def _detect_divergence(
        self, position: Position, data: pd.DataFrame, indicators: Dict
    ) -> bool:
    ...
    def create_hedge(
        self, position: Position, hedge_ratio: float, reason: str
    ) -> Optional[Position]:
    ...
    def _should_close_hedge(
        self, hedge: HedgePosition, market_data: Dict, indicators: Dict
    ) -> bool:
        """Determine if a hedge position should be closed."""
        try:
            # Minimum hedge duration
            if (
                datetime.now() - hedge.creation_time
            ).total_seconds() < self.min_hedge_duration * 60:
                return False
            ...
            if hedge.hedge_reason == "Strong trend reversal detected":
                if hedge.original_position.direction == "long":
                    ...
                    return (
                        indicators[symbol]["1h"]["macd_hist"][-1] > 0
                        and indicators[symbol]["5m"]["rsi"][-1] > 50
                    )
                else:
