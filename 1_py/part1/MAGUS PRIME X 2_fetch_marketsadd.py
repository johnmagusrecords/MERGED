...
if not all(
    [CAPITAL_API_KEY, CAPITAL_API_URL, CAPITAL_API_PASSWORD, CAPITAL_API_IDENTIFIER]
):
    raise ValueError(
...
def create_session():
    ...
    payload = {"identifier": CAPITAL_API_IDENTIFIER, "password": CAPITAL_API_PASSWORD}
    ...
    print(
        f"❌ Failed to create session. Response: {response.status_code}, {response.text}"
    )
    return None
...
def fetch_all_markets(session_tokens):
    ...
    if response.status_code == 200:
        ...
        print(
            f"✅ Successfully fetched markets! Total items: {len(data.get('nodes', []))}"
        )
        return data
    elif (
        response.status_code == 401
        and response.json().get("errorCode") == "error.invalid.session.token"
    ):
        ...
        print(
            f"❌ Failed to fetch markets. Response: {response.status_code}, {response.text}"
        )
        return None
