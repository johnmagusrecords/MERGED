...
# Initialize NewsApiClient (replace with your real API key)
newsapi = NewsApiClient(api_key="ea16bacd51c7462abcde520271143bf8")

# Example usage (you can place this where needed in bot.py):
...
if __name__ == "__main__":
    print("📰 Top headlines:")
    for title in fetch_latest_news():
        print("-", title)
...
def save_trade_to_csv(timestamp, symbol, action, tp, sl, result=None):
    ...
    df.to_csv(TRADE_HISTORY_FILE, mode='a', header=not os.path.exists(
        TRADE_HISTORY_FILE), index=False)
...
# Simulated stats (replace with real-time values from your bot)
def get_stats():
    return {
        "balance": "$12,345.67",
        "positions": 3,
...
def dashboard():
    """Main dashboard route for bot control and analytics."""
    ...
    pnl = sum([(float(t['TP']) - float(t['SL'])) if t.get('Result') ==
              'WIN' else -abs(float(t['SL'])) for t in trades if t.get('Result')])
...
def summary():
    """Summary route for quick bot stats."""
    stats = get_stats()
    return jsonify({
...
def find_market_match(symbol, markets):
    ...
    for market in markets:
        ...
        if normalized_symbol == market_name:
            logging.info(
                f"[Symbol Match] Exact match found: {symbol} -> {market['instrumentName']}")
            return market
    ...
    for market in markets:
        ...
        if normalized_symbol in market_name or market_name in normalized_symbol:
            logging.info(
                f"[Symbol Match] Partial match found: {symbol} -> {market['instrumentName']}")
            return market
    ...
    for market in markets:
        ...
        if normalized_symbol in epic or epic in normalized_symbol:
            logging.info(
                f"[Symbol Match] Epic match found: {symbol} -> {market['epic']}")
            return market
        attempted_matches.append(f"Epic: {epic}")

    # Log and save unmatched symbol
    logging.warning(
        f"[Symbol Match] No match found for {symbol} after all attempts")
...
class TradingBot:
    ...
    def get_available_markets(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                tradeable_markets = [m for m in markets if m.get(
                    'marketStatus') == 'TRADEABLE']
                logging.info(
                    f"Found {len(tradeable_markets)} tradeable markets")
    ...
    def analyze_news_sentiment(self, symbol):
        ...
        try:
            ...
            return sentiment_score / len(news['articles'])
    ...
    def get_historical_candles(self, epic, resolution='MINUTE_15', num_points=50):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.info(
                    f"Retrieved {len(df)} historical candles for {epic}")
                return df

            logging.error(
                f"Failed to fetch historical candles: {response.text}")
            return None
    ...
    def get_market_data(self, symbol):
        ...
        try:
            ...
            if search_response.status_code != 200:
                logging.error(
                    f"Failed to search markets: {search_response.text}")
                return None
            ...
            if not market:
                logging.error(
                    f"Could not find matching market for symbol: {symbol}")
                return None
                ...
                if pnl_percent > 0:
                    ...
                    if pnl_percent >= BREAKEVEN_TRIGGER * 100:
                        ...
                        if direction == 'BUY' and new_sl > stop_loss:
                            ...
                            send_telegram_message(msg)
                            logging.info(
                                f"Moving SL to breakeven for {symbol}")
                    ...
                    if pnl_percent >= TP_MOVE_PERCENT * 100:
                        ...
                        if direction == 'BUY':
                            ...
                            if new_tp > take_profit:
                                ...
                                send_telegram_message(msg)
                                logging.info(
                                    f"Trailing TP up for {symbol} to {take_profit:.2f}")
                        else:
                            new_tp = take_profit - tp_movement
                            if new_tp < take_profit:
                                take_profit = new_tp
                                ...
                                send_telegram_message(msg)
                                logging.info(
                                    f"Trailing TP down for {symbol} to {take_profit:.2f}")
                    ...
                    try:
                        ...
                        if action in ['BUY', 'SELL']:
                            ...
                            if success:
                                ...
                                send_telegram_message(msg)
                                logging.info(
                                    f"Trade executed successfully for {symbol}")
                            else:
                                logging.error(
