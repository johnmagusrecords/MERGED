...
class EnhancedMarketRegimeDetector:
    ...
    def detect_regime(self, symbol: str, data: pd.DataFrame) -> Tuple[MarketRegime, float]:
    ...
    def _determine_regime(self, hmm_state: int, kmeans_cluster: int,
        ...
        try:
            ...
            if volatility > 2.0:
                ...
                regime = MarketRegime.TRENDING_UP if price_direction > 0 else MarketRegime.TRENDING_DOWN
                ...
                confidence = min(confidence * 0.9, 1.0)  # Lower confidence for reversals
            else:
                regime = MarketRegime.RANGING
                confidence = min(confidence * 0.8, 1.0)  # Lower confidence for ranging markets
    ...
    def _detect_breakout(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            upper_breakout = prev_close <= upper.iloc[-2] and recent_close > upper.iloc[-1]
            lower_breakout = prev_close >= lower.iloc[-2] and recent_close < lower.iloc[-1]
    ...
    def _calculate_bollinger_width(self, prices: pd.Series, period: int = 20,
    ...
    def _check_reversal_patterns(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            bullish_engulfing = (
                close_prices.iloc[-2] < open_prices.iloc[-2] and  # Previous red candle
                close_prices.iloc[-1] > open_prices.iloc[-1] and  # Current green candle
            ...
            bearish_engulfing = (
                close_prices.iloc[-2] > open_prices.iloc[-2] and  # Previous green candle
                close_prices.iloc[-1] < open_prices.iloc[-1] and  # Current red candle
            ...
            # Check for doji
            doji = abs(close_prices.iloc[-1] - open_prices.iloc[-1]) <= (
