...
from market_regime import EnhancedMarketRegimeDetector, MarketRegime
...
class EnhancedTradingEngine:
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict:
        ...
        indicators['keltner_upper'], indicators['keltner_middle'], indicators['keltner_lower'] = self._calculate_keltner(high, low, close)
    ...
    def analyze_market(self, symbol: str) -> Tuple[str, Optional[Dict]]:
        ...
        try:
            ...
            long_conditions = (
                prediction.direction == "long" and
                prediction.confidence > 0.6 and
                indicators['rsi'][-1] < 70 and
                indicators['macd'][-1] > indicators['macd_signal'][-1] and
                current_price > indicators['sma_20'][-1] and
                indicators['adx'][-1] > self.trend_strength and
                indicators['mfi'][-1] < 80 and
                current_price > indicators['vwap'][-1] and
                indicators['cci'][-1] > -100 and
                confidence > 0.6
            )
            
            # Short setup conditions
            short_conditions = (
                prediction.direction == "short" and
                prediction.confidence > 0.6 and
                indicators['rsi'][-1] > 30 and
                indicators['macd'][-1] < indicators['macd_signal'][-1] and
                current_price < indicators['sma_20'][-1] and
                indicators['adx'][-1] > self.trend_strength and
                indicators['mfi'][-1] > 20 and
                current_price < indicators['vwap'][-1] and
                indicators['cci'][-1] < 100 and
                confidence > 0.6
            )
    ...
    def place_trade(self, symbol: str, direction: str, size: float,
    ...
    def close_trade(self, symbol: str, reason: str = ""):
        ...
        try:
            if symbol in self.active_trades:
                ...
                self.logger.info(f"Closing {trade.direction} trade for {symbol}. Reason: {reason}")
