...
class NewsAnalyzer:
            ...
            for article in articles['articles']:
                if article['title'] and article['description']:
                    ...
                    processed_articles.append({
                        ...
                        'impact': self._assess_news_impact(article['title'], article['description'])
                    })
    ...
    def _analyze_sentiment(self, text: str) -> Dict:
        ...
        try:
            ...
            # Custom word lists
            positive_words = {
                'bullish': 1.5, 'surge': 1.3, 'gain': 1.2, 'rally': 1.4,
                'breakthrough': 1.5, 'outperform': 1.3, 'upgrade': 1.2,
                'growth': 1.2, 'strong': 1.1, 'beat': 1.2, 'exceed': 1.3
            }
            
            negative_words = {
                'bearish': -1.5, 'crash': -1.5, 'plunge': -1.4, 'downgrade': -1.3,
                'recession': -1.4, 'crisis': -1.3, 'default': -1.2, 'miss': -1.1,
                'warning': -1.1, 'risk': -1.0, 'concern': -1.0, 'weak': -1.1
            }
            
            # Intensifiers and their multipliers
            intensifiers = {
                'very': 1.3, 'highly': 1.3, 'extremely': 1.5, 'significantly': 1.4,
                'substantially': 1.4, 'strongly': 1.3, 'major': 1.3, 'sharp': 1.3
            }
    ...
    def get_latest_news(self) -> List[Dict]:
        ...
        try:
            ...
            all_news.sort(key=lambda x: (
                ...
                1 if x['impact'] == 'medium' else 
                2 if x['impact'] == 'low' else 3,
                x['publishedAt']
            ), reverse=True)
