...
class AIPredictor:
    def __init__(self):
        ...
        self.feature_columns = ['open', 'high', 'low', 'close', 'volume', 'rsi', 'macd', 'signal', 'upper_bb', 'lower_bb']
        self.model_cache = {}
    ...
    def get_model_metrics(self, symbol: str) -> Dict:
        ...
        try:
            ...
            if os.path.exists(history_path):
                ...
                accuracy = history.get('accuracy', [])[-1] if history.get('accuracy') else 0
            else:
                accuracy = 0
    ...
    def cleanup_cache(self):
        ...
        expired_predictions = [
            ...
            if (current_time - datetime.strptime(key.split('_')[1], '%Y%m%d%H%M')).seconds > self.cache_ttl
        ]
        ...
        # Clean model cache
        expired_models = [
            symbol for symbol, data in self.model_cache.items()
            if (current_time - data['loaded_at']).seconds > 3600  # 1 hour TTL for models
        ]
