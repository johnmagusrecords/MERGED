...
def format_signal_message(pair, entry, stop, tp1, tp2, tp3, strategy, timeframe, mode, hold, commentary, asset_type, sentiment=None, news=None):
...
class SignalTemplates:
    ...
    def __init__(self, format_type="HTML"):
        ...
        self.enable_arabic = getattr(CONFIG, 'localization', {}).get('enable_arabic', False)
    ...
    def pre_signal_template(self, pair, direction, type_, timeframe, platform="Capital.com"):
...
"""
    ...
    def full_signal_template(self, pair, direction, entry, stop_loss,
                             tp1, tp2=None, tp3=None, timeframe="1h",
                             type_="Breakout", platform="Capital.com",
                             commentary=None, category=None):
        """
        ...
        if direction.upper() == "BUY":
            direction_formatted = f"BUY 🟢"
            arabic_direction = "شراء 🟢"
        else:
            direction_formatted = f"SELL 🔴"
...
✅ <b>هدف الربح 1:</b> {self.escape(tp1)}"""
    ...
    def recap_template(self, pair, direction, entry, stop_loss,
                       tp1, tp2=None, tp3=None,
                       hit_tp1=False, hit_tp2=False, hit_tp3=False,
                       hit_sl=False, pips=0, platform="Capital.com"):
        """
...
"""
    ...
    def recovery_template(self, pair, direction, entry, stop_loss,
                          tp1, tp2=None, tp3=None, timeframe="1h",
                          type_="Recovery", platform="Capital.com"):
...
✅ <b>هدف الربح 1:</b> {self.escape(tp1)}"""
    ...
    def news_alert_template(self, headline, assets_affected,
                            impact_level="High", source="News API"):
...
def get_pre_signal_message(pair, direction, type_, timeframe, platform="Capital.com"):
    ...
    return signal_templates.pre_signal_template(pair, direction, type_, timeframe, platform)


def get_signal_message(pair, direction, entry, stop_loss, tp1, tp2=None, tp3=None,
                       timeframe="1h", type_="Breakout", platform="Capital.com",
                       commentary=None, category=None):
...
def get_recap_message(pair, direction, entry, stop_loss, tp1, tp2=None, tp3=None,
                      hit_tp1=False, hit_tp2=False, hit_tp3=False, hit_sl=False,
                      pips=0, platform="Capital.com"):
    """Global function to get a signal recap message"""
    return signal_templates.recap_template(
        pair, direction, entry, stop_loss, tp1, tp2, tp3,
        hit_tp1, hit_tp2, hit_tp3, hit_sl, pips, platform
    )


def get_recovery_message(pair, direction, entry, stop_loss, tp1, tp2=None, tp3=None,
                         timeframe="1h", type_="Recovery", platform="Capital.com"):
    """Global function to get a recovery signal message"""
    return signal_templates.recovery_template(
        pair, direction, entry, stop_loss, tp1, tp2, tp3,
        timeframe, type_, platform
    )


def get_news_alert_message(headline, assets_affected, impact_level="High", source="News API"):
...
def generate_signal_message(signal_data, symbol=None, direction=None, **kwargs):
    """
    Generates a signal message based on the provided parameters.
    """
    ...
    if "stop_loss" in signal_data:
        message += f"Stop Loss: {signal_data['stop_loss']}\n"
    if "tp1" in signal_data:
        message += f"Take Profit 1: {signal_data['tp1']}\n"
    if "tp2" in signal_data:
        message += f"Take Profit 2: {signal_data['tp2']}\n"
    if "tp3" in signal_data:
        message += f"Take Profit 3: {signal_data['tp3']}\n"
    if "strategy" in signal_data:
        message += f"Strategy: {signal_data['strategy']}\n"
    if "asset_type" in signal_data:
        message += f"Asset Type: {signal_data['asset_type']}\n"
    if "holding_time" in signal_data:
        message += f"Holding Time: {signal_data['holding_time']}\n"
    if "commentary" in signal_data:
        message += f"Commentary: {signal_data['commentary']}\n"
    return message
...
def generate_recap_message(pair, result, exit_price, notes=None, entry=None, targets_hit=None):
    ...
    if not entry and not targets_hit:
        return signal_recap(result.lower() if result.lower() in ['tp1', 'tp2', 'tp3', 'sl', 'closed'] else "closed", pair)
...
def generate_messages(data_list):
    """Generate multiple messages from a list of data dictionaries"""
    return [generate_signal_message(data) for data in data_list]
...
if __name__ == "__main__":
    ...
    full_signal = get_signal_message(
        "BTCUSD", "BUY", 65000, 64000, 66000, 67000, 68000,
        "4h", "Breakout", "Capital.com", commentary, "Crypto"
    )
    print(full_signal)
    print("-" * 50)
    ...
    recovery = get_recovery_message(
        "BTCUSD", "BUY", 65500, 64500, 66500, 67500, 68500,
        "4h", "Breakout Recovery"
    )
    print(recovery)
    print("-" * 50)
