...
class TelegramNotifier:
    ...
    def __init__(self):
        ...
        if not self.token:
            logger.warning("Telegram bot token not found. Notifications will not be sent.")
            self.bot = None
            self.enabled = False
        else:
        ...
        self.admin_ids = [int(id) for id in os.getenv('TELEGRAM_ADMIN_IDS', '').split(',') if id]
...
"""
    ...
    def run_event_loop(self) -> None:
        ...
        # Test connection
        try:
            loop.run_until_complete(self.send_message_to_admins("Trading bot notification system is online."))
    ...
    def send_signal(self, symbol: str, direction: str, entry: float, 
                   take_profit: float, stop_loss: float, timeframe: str,
                   confidence: float, additional_info: str = "") -> None:
        ...
        self.send_warning(f"Incoming {direction.upper()} signal for {symbol} in 10 seconds", symbol, timeframe)
    ...
    def send_warning(self, warning_message: str, symbol: str = "", 
                    timeframe: str = "") -> None:
    ...
    def send_market_analysis(self, symbol: str, timeframe: str, price: float,
                           trend: str, sentiment: str, key_points: List[str]) -> None:
...
"""
    ...
    def get_subscribers_count(self) -> int:
        ...
        return sum(1 for user_data in self.user_ids.values() if user_data.get('subscribed', False))
